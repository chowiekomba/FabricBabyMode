...
All I had to do was check if the player's current health was less than his max health. If so, give saturation.

# The nineth feature (v0.2.3)
This update was a doozy. To get the mobs to not be able to attack the player, I had to make a box around the player, and check if there's any entities in that box. If there is, I had to check if they were hostile. If so, I would create a string variable that would kill the mob.

# The tenth feature (v0.2.4)
This feature was pretty easy. To get the player not be able to die in lava, I checked if the player's feet was of lava. If so, apply fire resistance potion. Very similar to the water feature.
...
I felt like the mod was a bit boring and was a bit to serious, so I added a sign to be placed after the mob dies that says "{mob} died to: your existence" This was an unexpectedly hard feature cause for some reason the mob would move 2 blocks before being killed so there would be 2-3 signs that would be placed. I had to add one more command. It was making the entity have no ai by merging the data. That way it wouldn't move when the sign would be placed. Then it would place the sign, and after kill the entity.

# the fourteenth feature (v0.2.4.4)
This update was very small. All I did was make it so everytime you load into the world it sets commandFeedback to false so the command outputs don't fill up the chat.

# the fiveteenth feature (v0.2.5)
This update was a lot of trial and error. The actual concept is pretty simple, if the player is in the air, apply the slow falling effect. It wasn't actually that easy. I figured out that sprint jumping and stuff isn't optimal with slow falling so I had to make it be applied for a very short amount of time **and** have it not be applied unless you actually will take fall damage. I figured out you can get the player's y velocity and after a lot of trial and error, I figured out -0.66 was the max velocity you can have before taking fall damage.
...
This update makes it so every effect that you get is infinite (if its a positive effect). I still left out feather falling though becuase of the problems listed earlier in v0.2.5.

# the twenty-first feature (v0.3.1)
This update was a bit bigger than others, but that was also because I didn't want to screw up my versioning. I made it so every armor has max enchants, and made it so diamond ore drops a bunch of diamonds and has a chance of dropping netherite. I felt bad for someone if they actually went mining so I left a big suprise if they found diamonds.
...
# the twenty-fiveth feature (v0.3.5)
...
I messed up the versioning. I didn't realize how far ahead my features were compared to my version number lol. I guess this is why you should use a version road map. Anyway, this update makes it so when you get within 5 blocks of the ender dragon, it will say something, and die. Allowing you to skip the fight.

# the twenty-eighth feature (v0.8)
This update was a bit hard, because when I initially implemented the feature, when you fell into the void, it would teleport you back up, but it would reset your camera position. So the real challenging part was preserving the player's camera rotation (especially in the end).
